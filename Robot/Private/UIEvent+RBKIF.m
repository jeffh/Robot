/* Taken from KIF to support iOS 8.3
 */
...
//
@interface RBGSEventProxy : NSObject
{
@public
    unsigned int flags;
    unsigned int type;
...
- (void)RB_setEventWithTouches:(NSArray *)touches
{
    ...
    if ([NSProcessInfo instancesRespondToSelector:@selector(isOperatingSystemAtLeastVersion:)]
        ...
        [self RB_setIOHIDEventWithTouches:touches];
    } else {
        [self RB_setGSEventWithTouches:touches];
    }
}

- (void)RB_setGSEventWithTouches:(NSArray *)touches
{
    UITouch *touch = touches[0];
    CGPoint location = [touch locationInView:touch.window];
    RBGSEventProxy *gsEventProxy = [[RBGSEventProxy alloc] init];
...
- (void)RB_setIOHIDEventWithTouches:(NSArray *)touches
{
