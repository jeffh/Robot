...
SPEC_BEGIN(RBCollectionViewCellsProxySpec)

describe(@"RBCollectionViewCellsProxy", ^{
    __block RBCollectionViewCellsProxy *subject;
    __block SampleViewController *controller;

    beforeEach(^{
        controller = [[SampleViewController alloc] init];
        [controller fillCollectionViewWithNumberOfRows:5 andSections:2];
        controller.view should_not be_nil;

        subject = [RBCollectionViewCellsProxy cellsFromCollectionView:controller.collectionView];
    });
